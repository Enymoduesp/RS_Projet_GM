...
friendship_t * scan_friendship(list_t * Lpers){
	...
	F->B = P2;
	// A aime B donc on met B dans la liste d'amis de A
	queue(P1->friends, P2);
	return F;
}
...
void free_friendship(friendship_t ** ptrFriend, bool del_persons){
	...
	if (del_persons) {
		// del_friends=false : les personnes existent encore dans Lpers
		free_person(&(*ptrFriend)->A, false);
		free_person(&(*ptrFriend)->B, false);
	}
	free(*ptrFriend);
	*ptrFriend = NULL;
}
