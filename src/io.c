...
person_t * person_from_stream ( FILE * stream, stream_mode_t mode ) {
	...
	if(mode == TEXT){
		...
		if(fscanf(stream, "%s %s %d/%d/%d",
			...
			&(P->birth_date.month),
			&(P->birth_date.year)) != 5){
			free_person(&P, true); // new_person() a créé P->friends, il faut tout libérer
			return NULL;
		}
	}
	else{
		...
		if(fread(P, sizeof(person_t), 1, stream) != 1){
			free_person(&P, true); // même raison
			return NULL;
		}
...
friendship_t * friendship_from_stream(FILE * stream, stream_mode_t mode, list_t * Lpers){
	...
	if(mode == TEXT){
		// %*s lit et jette le mot "aime" qui sépare les deux noms dans le fichier
	...
	F->A = A;
	F->B = B;
	// A aime B donc on ajoute B dans la liste d'amis de A
	queue(A->friends, B);
